...
define([
    ...
    'SixtySeven_UploadVideo/js/get-video-information'
], function ($, _) {
    'use strict';

    $.widget('mage.createUploadedVideoPlayer', {
        options: {
            videoId: '',
            videoProvider: 'external-uploaded-video',
        ...
        _doUpdate: function () {
           ...
           // this.element.find(this.options.metaData.DOM.duration).text(this.options.metaData.data.duration);
            ...
            this.element.find('.' + this.options.videoClass).uploadedProductVideoLoader();
    ...
    $.widget('mage.updateInputFields', {
        options: {
            reset: false,
            DOM: {
                urlField: 'input[name="uploaded_video_url"]',
                titleField: 'input[name="uploaded_video_title"]',
                fileField: '#uploaded_file_name',
                descriptionField: 'textarea[name="uploaded_video_description"]',
                thumbnailLocation: '.field-new_uploaded_video_screenshot_preview .admin__field-control'
            },
            data: {
                url: '',
                title: '',
    ...
    /**
     */
    $.widget('mage.newUploadVideoDialog', {
        ...
        _videoPreviewInputSelector: '#new_uploaded_video_screenshot',

        _videoPreviewRemoteSelector: '',

        _videoDisableinputSelector: '#new_uploaded_video_disabled',

        _videoPreviewImagePointer: '#new_uploaded_video_screenshot_preview',

        _videoFormSelector: '#new_uploaded_video_form',

        _itemIdSelector: '#uploaded_item_id',

        _videoUrlSelector: '[name="uploaded_video_url"]',

        _videoImageFilenameselector: '#uploaded_file_name',
      ...
      //  _videoInformationBtnSelector: '[name="new_video_get"]',
        ...
        _bind: function () {
            ...
            //alert('sdfsdfasdfasdfasdfsd');
            this._videoUrlWidget = this.element.find(this._videoUrlSelector).toUploadVideoData({
                eventSource: 'change',
           ...
           // this._videoInformationGetBtn.on('click', $.proxy(this._onGetVideoInformationClick, this));
            this._videoInformationGetUrlField.on('change', $.proxy(this._onGetVideoInformationFocusOut, this));
            ...
            this._videoUrlWidget.on('finish_video_upload', $.proxy(this._onFinishedVideoUpload, this));
         ...
         * @private
         */
        _onFinishedVideoUpload: function (e, data) {
            var self = this; 
        ...
        _onGetVideoInformationSuccess: function (e, data) {
            ...
            self.element.on('finish_update_video finish_create_video', $.proxy(function (element, playerData) {
                    if (!self._onlyVideoPlayer ||
                        ...
                        this._loadRemotePreview(this.options.placeholderDefault);
                    }
                    self._onlyVideoPlayer = true;
                }, this))
                .createUploadedVideoPlayer({
        ...
        _loadRemotePreview: function (sourceUrl) {
            ...
            $.ajax({
                ...
                success: $.proxy(function (result) {
                    ...
                    this._getPreviewImage().css('width','auto');
        ...
        _uploadVideo: function (file, oldFile, callback) {
            var url = this.options.uploadVideoUrl,
            data = {
                files: file,
                url: url
            };
            ...
            this._blockActionButtons(true, true);
            this._uploadVideoFile(data, $.proxy(function (result) {
         ...
         * @private
         */
        _onVideoUploaded: function (result, file, oldFile, callback) {
            var data = JSON.parse(result);

            if (this.element.find('#uploaded_video_url').parent().find('.image-upload-error').length > 0) {
                this.element.find('.image-upload-error').remove();
            }

            if (data.errorcode || data.error) {
                this.element.find('#uploaded_video_url').parent().append('<div class="image-upload-error">' +
            ...
            this.element.find(this._videoFormSelector).find('input[name="uploaded_file_path"]').val(data['uploaded_file_path']);
        ...
        _onImageLoaded: function (result, file, oldFile, callback) {
            ...
            $.each(this.element.find(this._videoFormSelector).serializeArray(), function (i, field) {
                ...
                if(field.name=='uploaded_file_path'){
                    uploadedfilepath = field.value;
                }
                var nm = field.name+"";
                
                nm = nm.replace('uploaded_', '');
                data[nm] = field.value;
            });
            data['video_url'] = uploadedfilepath; 
            data['media_type'] = 'uploaded_video'; 
         ...
         * @private
         */
        _uploadVideoFile: function (data, callback) {
            var fu = this.element.find(this._videoUrlSelector),
                tmpInput = document.createElement('input'),
                fileUploader = null;
        ...
        _create: function () {
            ...
            for (i = 0; i < imgs.length; i++) {
                ...
                if (tmp['media_type'] === 'external-uploaded-video') {
            ...
            this.element.modal({
                ...
                modalClass: 'mage-new-video-dialog upload-new-video form-inline',
                title: $.mage.__('Upload New Video'),
                buttons: [
                    {
                        text: $.mage.__('Save'),
                ...
                opened: function () {
                    ...
                    widget.element.find('#uploaded_video_url').focus();
                    ...
                    if (!file) {
                        ...
                        modalTitleElement.text($.mage.__('Upload New Video'));
        ...
        isValid: function (callback) {
            ...
            this._videoUrlWidget.trigger('validate_video_url', $.proxy(function () {
                ...
                var videoValidStatus = videoForm.valid() && videoLoaded;
                callback.call(0,videoValidStatus);
            }, this));
        ...
        _onOpenDialog: function (e, imageData) {
            ...
            if (imageData['media_type'] === 'external-uploaded-video') {
                this.imageData = imageData;
                modal.find('.video-create-button').hide();
                modal.find('.video-delete-button').show();
                modal.find('.video-edit').show();
                modal.createUploadedVideoPlayer({
                    reset: true
                }).createUploadedVideoPlayer('reset');
        ...
        toggleButtons: function () {
            ...
            modal.find('.video-placeholder, .add-video-button-container > button').click(function () {
                ...
                modal.createUploadedVideoPlayer({
                    reset: true
                }).createUploadedVideoPlayer('reset').updateInputFields({
                    reset: true
            ...
            this._gallery.on('click', '.item.video-item', function () {
                ...
                modal.find('.video-delete-button').show();
                modal.find('.video-edit').show();
                modal.find('.mage-new-video-dialog').createUploadedVideoPlayer({
                    reset: true
                }).createUploadedVideoPlayer('reset');
            });
    ...
    return $.mage.newUploadVideoDialog;
});
