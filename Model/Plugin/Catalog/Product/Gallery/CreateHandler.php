...
use SixtySeven\UploadVideo\Model\Product\Attribute\Media\ExternalVideoEntryConverter;
use SixtySeven\UploadVideo\Setup\InstallSchema;
...
class CreateHandler extends AbstractHandler
{
    ...
    protected function saveVideoData(array $videoDataCollection, $storeId)
    {
        foreach ($videoDataCollection as $item) {
            ...
            if($tempstr=='tmp'){
                ...
                $url = implode('.', $urlArr);
                $newurl = $this->moveVideoFromTmp($url);
                if($newurl){
                   $item['video_url'] =  $newurl;
                } else {
                    continue;
                }
            }
    ...
    /**
     * moveVideoFromTmp move video from temp to catalog.product
     * @param  string $filepath temp file path
     * @return string|bool  newfilepath or false if error
     */
    protected function moveVideoFromTmp($filepath)
    { 
        try{
            $result = $this->imageUploader->moveFileFromTmp($filepath);
            return $result;
        } catch (\Exception $e){
            return false;
        }
